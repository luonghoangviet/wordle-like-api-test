...
def untrusted_request(context, request_verb, url_path_segment):
    ...
    context.response = getattr(requests, request_verb.lower())(url, headers=context.headers, verify=False)
...
def request_with_parameters(context, request_verb, url_path_segment):
    ...
    context.response = getattr(requests, request_verb.lower())(url, params, headers=context.headers, verify=context.verify_ssl)
...
def request_with_parameters(context, request_verb, url_path_segment):
    ...
    context.response = getattr(requests, request_verb.lower())(url, params, headers=context.headers, verify=False)
...
def status_code_validation(context, invalid_http_status_code):
    nose.tools.assert_not_equal(context.response.status_code, int(invalid_http_status_code))
...
def status_code_array_validation(context, expected_http_status_codes):
    ...
    nose.tools.assert_in(context.response.status_code, expected_codes_list)
...
def parameter_validation(context, header_name, expected_header_value):
    nose.tools.assert_equal(context.response.headers[header_name], str(expected_header_value))
